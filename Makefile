...
.PHONY: install
## `install`: Fetch dependencies needed to run `crcgen`
install:
...
.PHONY: run
## :
## `run`: Run `crcgen` in production mode
...
.PHONY: run-debug
## `run-debug`: Run `crcgen` in debug mode
...
.PHONY: docker-gen
## :
## `docker-gen`: Create a production docker image for `crcgen`
docker-gen:
...
.PHONY: docker-debug
## `docker-debug`: Create debug-friendly docker images for `crcgen`
docker-debug:
	echo "Building docker image \`$(IMAGE):$(VERSION)\`..."
	docker build --rm=false \
		--build-arg final_image=golang:1.17 \
